...
const AUTOMATION_URLS = {
  ...
  // Google Drive folder
  "data-validation":
    "https://drive.google.com/drive/folders/12sfBN4dYSlZJmNs1HmGrmssKZFLfPf4r?usp=sharing",
};
...
const translations = {
  en: {
    ...
    messages: {
      success:
        "Your request has been submitted successfully! You will receive the results via email.",
      error:
        "An error occurred. Please try again or contact us at gonzalo@menatech.cloud",
  ...
  es: {
    ...
    messages: {
      success:
        "¡Tu solicitud ha sido enviada exitosamente! Recibirás los resultados por correo electrónico.",
      error:
        "Ocurrió un error. Por favor intenta de nuevo o contáctanos en gonzalo@menatech.cloud",
...
function openAutomation(automationType) {
  ...
  if (form) {
    form.addEventListener("submit", (e) =>
      handleAutomationSubmit(e, automationType),
    );
  }
...
async function handleFeedbackSubmit(event) {
  ...
  try {
    ...
    showMessage(
      "error",
      error.message || translations[currentLang].messages.error,
    );
  } finally {
    submitBtn.classList.remove("loading");
    submitBtn.innerHTML = originalText;
